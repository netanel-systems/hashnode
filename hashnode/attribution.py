"""Attribution join logic — Hashnode platform.
...
def attribute_follow(
    ...
    # Filter entries where target_username matches the new follower
    matches = [
        e for e in entries
        if e.get("target_username", "").lower() == follower_username.lower()
    ]
...
def calculate_fbr(data_dir: Path, lookback_days: int = 7) -> dict:
    ...
    # Get unique users we engaged with
