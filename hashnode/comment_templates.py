"""Comment template categories for Hashnode — prompt-injection guidance.
...
Platform-specific constraints for Hashnode:
- 2-4 sentences
- Max 600 characters
- Standard markdown

Schema version: H3 (GitLab #14)
"""
...
def pick_template_category(last_category: str | None = None) -> str:
    ...
    if not available:
        # Fallback: all categories are the same as last (should not happen with 4)
...
def has_question(text: str) -> bool:
    ...
    Args:
        text: The comment or reply text to check.
    ...
    if "?" in text:
        return True
    # Check for question words at sentence starts
    question_patterns = [
    ...
    for pattern in question_patterns:
        if re.search(pattern, text.lower()):
            # Only count if it reads like a question (starts a clause)
            # Simple heuristic: question word at start of sentence
